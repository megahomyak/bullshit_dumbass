...
let matchCommand = async (command, matchers) => {
    for (let matcher of matchers) {
        ...
        let argsString = argsMatched.map(a => `\\s+(${a.regexp})`);
        ...
        await fsPromises.writeFile(filepath, await textCompleter([
            {
                ...
                content: [
                    ...
                    "* voice {panning, from -1 for \"all to left\" to 1 for \"all to right\"} {phrase} - starts a voice from the character, goes to later commands only after the voice ends",
                    "* sound {panning, from -1 for \"all to left\" to 1 for \"all to right\"} {duration in seconds} {sound description} - starts a sound, goes to later commands only after the sound ends",
                    "* bgstart {panning, from -1 for \"all to left\" to 1 for \"all to right\"} {sound description} - starts playing a background sound, goes to later commands immediately",
            ...
            return async () => {
                ...
                if (remainderDuration != 0) {
                    infilesShort.push({
                        ...
                        start: infilesShort.length * oneDuration,
                    });
                }
                infiles.push(...infilesShort.map(infileShort => ({
                    path: filepath,
                    pan: panning,
                    start: infileShort.start,
        ...
        for (let line of formattedStory.trim().split("\n")) {
            matchCommand(line, [
                ["voice number rest", async (panning, phrase) => {
                    let filepath = await make(voices.file(makeFileName(phrase)), async filepath => {
